...
class GLStandardGeomsPass extends GLPass {
  __dirtyItemIndices: any;
  __drawItems: any;
  __drawItemsIndexFreeList: any;
  __drawItemsTexture: any;
  /**
   * Create a GL pass.
   */
  constructor() {
   ...
   */
  itemAddedToScene(treeItem: any, rargs: any) {
    if (treeItem instanceof GeomItem) {
      const geomItem = treeItem
      if (!geomItem.getMetadata('glgeomItem')) {
        const checkGeom = (geomItem: any) => {
   ...
   * @return {Boolean} - The return value.
   */
  itemRemovedFromScene(treeItem: any, rargs: any) {
    // @ts-expect-error ts-migrate(2552) FIXME: Cannot find name 'geomItem'. Did you mean 'GeomIte... Remove this comment to see the full error message
   ...
   */
  constructShaders(shaderName: any) {
  ...
  addGeomItem(geomItem: any) {
    ...
    glgeomItem.on('updated', (event: any) => {
      if (!event) {
   ...
   */
  removeGLGeom(geomItemMapping: any, materialGeomMapping: any) {
   ...
   * @private
   */
  __populateDrawItemDataArray(geomItem: any, index: any, dataArray: any) {
   ...
   * @return {any} - The return value.
   */
  bindShader(renderstate: any, glshader: any) {
   ...
   * @return {any} - The return value.
   */
  bindMaterial(renderstate: any, glmaterial: any, warnMissingUnifs: any) {
