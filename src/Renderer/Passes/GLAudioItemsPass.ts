...
class GLAudioItemsPass extends GLPass {
     ...
     */
    // @ts-expect-error ts-migrate(2416) FIXME: Property 'itemAddedToScene' in type 'GLAudioItemsP... Remove this comment to see the full error message
     ...
     * @return {Boolean} - The return value.
     */
    // @ts-expect-error ts-migrate(2416) FIXME: Property 'itemRemovedFromScene' in type 'GLAudioIt... Remove this comment to see the full error message
     ...
     */
    addAudioSource(treeItem: any, audioSource: any, parameterOwner: any) {
        ...
        if (spatializeParam && spatializeParam.getValue() == false) {
            ...
            const updatePannerNodePosition = () => {
                ...
                let mat4;
                if (treeItem instanceof GeomItem)
                    mat4 = treeItem.getGeomMat4();
                else
                    mat4 = treeItem.getParameter('GlobalXfo').getValue().toMat4();
            ...
            treeItem.on('globalXfoChanged', (event: any) => {
                updatePannerNodePosition();
            });
        }
...
if (window.navigator) {
    (window as any).ZeaAudioaudioCtx = audioCtx;
}
