...
class GLBillboardsPass extends GLPass {
  __atlas: any;
  __billboardDataArray: any;
  __billboards: any;
  __dirtyBillboards: any;
  __drawCount: any;
  ...
  __glshader: any;
  __indexArray: any;
  __instanceIdsBuffer: any;
  __modelMatrixArray: any;
  __prevSortCameraPos: any;
  __renderer: any;
  __shaderBinding: any;
  __threshold: any;
  __tintColorArray: any;
  __updateRequested: any;
  __width: any;
  indexArrayUpdateNeeded: any;
  updateIndexArrayId: any;
  /**
  ...
  init(renderer: any, passIndex: any) {
    ...
    // @ts-expect-error ts-migrate(2554) FIXME: Expected 2-4 arguments, but got 5.
    this.__atlas = new GLImageAtlas(this.__renderer.gl, 'Billboards', 'RGBA', 'UNSIGNED_BYTE', [1, 1, 1, 0])
    const emitUpdated = (event: any) => this.emit('updated', event)
   ...
   * @private
   */
  __populateBillboardDataArray(billboardData: any, index: any, dataArray: any) {
  ...
  __updateBillboards() {
    const doIt = () => {
      ...
      if (!gl.floatTexturesSupported || !gl.drawElementsInstanced) {
        ...
        this.__indexArray.forEach((index: any) => {
          // if (index == -1) return;
          const billboardData = this.__billboards[index]
          const billboard = billboardData.billboard
          const mat4 = billboard.getParameter('GlobalXfo').getValue().toMat4()
  ...
  sort(cameraPos: any) {
    ...
    this.__indexArray.sort((a: any, b: any) => {
      if (a == -1) return 1
      if (b == -1) return -1
  ...
  draw(renderstate: any) {
    ...
    if (this.__dirtyBillboards.size > 0) {
      this.__dirtyBillboards.forEach((index: any) => {
