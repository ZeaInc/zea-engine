...
class GLTexture2D extends RefCounted {
  ...
  __loaded: any;
  __magFilter: any;
  __magFilterParam: any;
  __minFilter: any;
  __minFilterParam: any;
  __mipMapped: any;
  __texture: any;
  __type: any;
  __typeParam: any;
   ...
   */
  configure(params: any, emit = true) {
      ...
      if (gl.name == 'webgl2') {
        ...
        if (gl.__ext_float) {
          ...
          if (gl.__ext_half_float) {
            // @ts-expect-error ts-migrate(2588) FIXME: Cannot assign to 'type' because it is a constant.
            type = 'HALF_FLOAT'
  ...
  clear() {
    ...
    switch (this.__type) {
      case gl.UNSIGNED_BYTE:
        // @ts-expect-error ts-migrate(2552) FIXME: Cannot find name 'UInt8Array'. Did you mean 'Uint8... Remove this comment to see the full error message
        data = new UInt8Array(numPixels * numChannels)
        break
      case gl.HALF_FLOAT:
        // @ts-expect-error ts-migrate(2552) FIXME: Cannot find name 'UInt16Array'. Did you mean 'Uint... Remove this comment to see the full error message
   ...
   * @param {boolean} emit - The emit value.
   */
  resize(width: any, height: any, preserveData = false, emit = true) {
    const gl = this.__gl
   ...
   * @param {boolean} bind - The bind value
   */
  populate(dataArray: any, width: any, height: any, offsetX = 0, offsetY = 0, bind = true) {
    const gl = this.__gl
