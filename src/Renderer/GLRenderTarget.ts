...
class GLRenderTarget extends EventEmitter {
    ...
    configure(params: any) {
        ...
        this.textureTargets.forEach((colorTexture: any) => {
        ...
        this.params = p;
        this.type = (p as any).type;
        this.format = (p as any).format;
        this.internalFormat = (p as any).internalFormat;
        this.filter = (p as any).filter;
        this.wrap = (p as any).wrap;
        this.flipY = (p as any).flipY;
        this.width = p.width;
        ...
        const numColorChannels = params.numColorChannels != undefined ? params.numColorChannels : (p as any).format != undefined ? 1 : 0;
        for (let i = 0; i < numColorChannels; i++) {
            gl.activeTexture(gl.TEXTURE0 + 1);
            const colorTexture = gl.createTexture();
            gl.bindTexture(gl.TEXTURE_2D, colorTexture);
            gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, (p as any).wrapS);
            gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, (p as any).wrapT);
            gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, (p as any).minFilter);
            gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, (p as any).magFilter);
        ...
        if ((p as any).depthFormat) {
            ...
            gl.texImage2D(gl.TEXTURE_2D, 0, (p as any).depthInternalFormat, p.width, p.height, 0, (p as any).depthFormat, (p as any).depthType, null);
        }
     ...
     */
    bindForWriting(renderstate: any, clear = false) {
        if (renderstate) {
     ...
     * @param {any} renderstate - The renderstate value.
     */
    unbindForWriting(renderstate: any) {
        if (renderstate)
     ...
     */
    bindColorTexture(renderstate: any, unif: any, channelId = 0) {
        const gl = this.__gl;
     ...
     * @return {boolean} - The return value.
     */
    bindDepthTexture(renderstate: any, unif: any) {
        const gl = this.__gl;
        const unit = renderstate.boundTextures++;
        gl.uniform1i(unif.location, unit);
        gl.activeTexture(gl.TEXTURE0 + unit);
     ...
     */
    resize(width: any, height: any, preserveData = false) {
        const gl = this.__gl;
