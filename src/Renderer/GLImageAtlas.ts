...
class GLImageAtlas extends GLRenderTarget {
    __async: any;
    __atlasLayoutTexture: any;
    __formatParam: any;
    __gl: any;
    __layout: any;
    __layoutNeedsRegeneration: any;
    __layoutVec4s: any;
    __name: any;
    __subImages: any;
    __typeParam: any;
     ...
     */
    constructor(gl: any, name: any, format = 'RGBA', type = 'FLOAT') {
        // @ts-expect-error ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        super(gl);
        this.__name = name;
    ...
    generateAtlasLayout() {
        ...
        const blocks: any = [];
        this.__subImages.forEach((subImage: any, index: any) => {
            blocks.push({
        ...
        if (!gl.floatTexturesSupported) {
            this.__layoutVec4s = [];
            this.__layout.forEach((layoutItem: any, index: any) => {
    ...
    renderAtlas(cleanup = false, off = 0) {
        ...
        const unifs = (renderstate as any).unifs;
        for (let j = off; j < this.__subImages.length; j++) {
            ...
            (renderstate as any).boundTextures--;
        }
        if (cleanup) {
            this.cleanup();
        }
     ...
     * @return {any} - The return value.
     */
    bindToUniform(renderstate: any, unif: any) {
