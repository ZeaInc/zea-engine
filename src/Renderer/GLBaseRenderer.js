...
class GLBaseRenderer extends ParameterOwner {
   ...
   *
   * @return {function} - The teardown function.
   */
  setupWebGL($canvas, webglOptions) {
    ...
    if (tagName !== 'CANVAS') {
      throw new Error(
        `Using \`${tagName}\` as root element is not valid. Use a \`CANVAS\` instead. See: https://docs.zea.live/zea-engine/#/getting-started/get-started-with-engine?id=basic-setup`
      )
    }

    this.__glcanvas = $canvas

    if (window.getComputedStyle(this.__glcanvas).position === 'static') {
      throw new Error(
        `The \`CANVAS\` element's position must be other than \`static\`. See: https://docs.zea.live/zea-engine/#/getting-started/get-started-with-engine?id=basic-setup`
      )
    }
    ...
    resizeObserver.observe(this.__glcanvas.parentElement)
