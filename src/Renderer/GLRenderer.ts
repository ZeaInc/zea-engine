...
import { BackgroundImageShader, OctahedralEnvMapShader, LatLongEnvMapShader, SterioLatLongEnvMapShader, DualFishEyeToLatLongBackgroundShader, } from './Shaders/EnvMapShader.js';
import { generateShaderGeomBinding } from './GeomShaderBinding.js';
import { OutlinesShader } from './Shaders/OutlinesShader.js';
import { GLMesh } from './GLMesh.js';
// @ts-expect-error ts-migrate(2307) FIXME: Cannot find module './logo-zea.svg' or its corresp... Remove this comment to see the full error message
...
class GLRenderer extends GLBaseRenderer {
    ...
    __backgroundMapShader: any;
    __backgroundMapShaderBinding: any;
    __cutPlaneNormal: any;
    __debugMode: any;
    __debugTextures: any;
    __displayEnvironment: any;
    __exposure: any;
    __fbo: any;
    __fwBuffer: any;
    __gamma: any;
    __glBackgroundMap: any;
    __glEnvMap: any;
    __highlightedGeomsBuffer: any;
    __highlightedGeomsBufferFbo: any;
    __outlineShader: any;
    __rayCastRenderTarget: any;
    __rayCastRenderTargetProjMatrix: any;
    __tonemap: any;
    _planeDist: any;
    quad: any;
    rayCastArea: any;
    ...
    constructor($canvas: any, options = {}) {
        ...
        if (!(options as any).disableTextures) {
            // @ts-expect-error ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        ...
        if (!(options as any).hideSplash) {
            const setupSplashScreen = (holdTime: any) => {
        ...
        if (env instanceof EnvMap) {
            ...
            if (!this.__glEnvMap) {
                if (env.type === 'FLOAT') {
                    ...
                    // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'GLImageStream'.
     ...
     * @return {any} - The return value.
     */
    raycastWithRay(ray: any, dist: any, area = 0.01, mask = ALL_PASSES) {
        const xfo = new Xfo();
        // @ts-expect-error ts-migrate(2554) FIXME: Expected 3 arguments, but got 2.
        xfo.setLookAt(ray.start, ray.start.add(ray.dir));
        return this.raycast(xfo, ray, dist, area, mask);
    }
    raycastWithXfo(xfo: any, dist: any, area = 0.01, mask = ALL_PASSES) {
     ...
     * @return {any} - The return value.
     */
    raycast(xfo: any, ray: any, dist: any, area = 0.01, mask = ALL_PASSES) {
        const gl = this.__gl;
        ...
        // Then top bottom (1, 7)
        const checkPixel = (id: any) => geomDatas[id * 4 + 3] != 0;
     ...
     * @return {any} - The return value.
     */
    raycastCluster(xfo: any, ray: any, dist: any, area = 0.01, mask = ALL_PASSES) {
        const gl = this.__gl;
        if (!this.__rayCastRenderTarget) {
            this.__rayCastRenderTarget = new GLRenderTarget(gl, {
                type: 'FLOAT',
     ...
     */
    drawBackground(renderstate: any) {
        if (this.__glBackgroundMap) {
     ...
     * @param {any} renderstate - The renderstate value.
     */
    bindGLRenderer(renderstate: any) {
