...
let activeGLRenderer: any = undefined;
...
class GLBaseRenderer extends ParameterOwner {
    __$canvas: any;
    __activeViewport: any;
    __continuousDrawing: any;
    __drawSuspensionLevel: any;
    __floatGeomBuffer: any;
    __gizmoContext: any;
    __gizmoPass: any;
    __gl: any;
    __glcanvas: any;
    ...
    __passCallbacks: any;
    __passes: any;
    __passesRegistrationOrder: any;
    __preproc: any;
    __redrawRequested: any;
    __renderGeomDataFbosRequested: any;
    __scene: any;
    __screenQuad: any;
    __shaderDirectives: any;
    __shaders: any;
    __supportXR: any;
    __viewports: any;
    __xrViewport: any;
    __xrViewportPresenting: any;
    __xrViewportPromise: any;
    /**
    ...
    constructor($canvas: any, options = {}) {
        ...
        this.setupWebGL($canvas, (options as any).webglOptions ? (options as any).webglOptions : {});
        ...
        // ////////////////////////////////////////////
        // WebXR
        this.__supportXR = (options as any).supportXR !== undefined ? (options as any).supportXR : true;
        ...
        this.__xrViewportPromise = new Promise((resolve, reject) => {
            if (this.__supportXR) {
                ...
                if ((navigator as any).xr) {
                    ...
                    if ((navigator as any).xr.supportsSessionMode) {
                        // Old
                        (navigator as any).xr
     ...
     */
    addShaderPreprocessorDirective(name: any, value: any) {
        if (value)
     ...
     * @return {GLViewport} - The return value.
     */
    getViewportAtPos(offsetX: any, offsetY: any) {
     ...
     * @param {number} offsetY - The viewport offset in the Y axis.
     */
    activateViewportAtPos(offsetX: any, offsetY: any) {
        if (this.__xrViewportPresenting)
            return;
    ...
    resumeDrawing() {
        ...
        if (this.__drawSuspensionLevel == 0) {
            if (this.__loadingImg)
                this.__$canvas.removeChild(this.__loadingImg);
     ...
     * @deprecated
     */
    setupGrid(gridSize: any, gridColor: any, resolution: any, lineThickness: any) {
     ...
     */
    assignTreeItemToGLPass(treeItem: any) {
        let handled = false;
     ...
     */
    resizeFbos(width: any, height: any) { }
    /**
     ...
     */
    setupWebGL($canvas: any, webglOptions: any) {
    ...
    bindEventHandlers() {
        ...
        const prepareEvent = (event: any) => {
            event.propagating = true;
            event.stopPropagation = () => {
                event.propagating = false;
            };
        };
        const calcRendererCoords = (event: any) => {
        ...
        /** Mouse Events Start */
        this.__glcanvas.addEventListener('mousedown', (event: any) => {
            prepareEvent(event);
        ...
        document.addEventListener('mouseup', (event) => {
            ...
            const viewport = activeGLRenderer.getActiveViewport();
            if (viewport) {
                (event as any).pointerType = POINTER_TYPES.mouse;
        ...
        document.addEventListener('mousemove', (event) => {
            ...
            calcRendererCoords(event);
            if (!pointerIsDown)
                activeGLRenderer.activateViewportAtPos((event as any).rendererX, (event as any).rendererY);
            const viewport = activeGLRenderer.getActiveViewport();
            if (viewport) {
                (event as any).pointerType = POINTER_TYPES.mouse;
        ...
        this.__glcanvas.addEventListener('mouseenter', (event: any) => {
            if (!pointerIsDown) {
                activeGLRenderer = this;
                event.pointerType = POINTER_TYPES.mouse;
                prepareEvent(event);
        ...
        this.__glcanvas.addEventListener('mouseleave', (event: any) => {
            if (activeGLRenderer != this || !isValidCanvas())
                return;
            prepareEvent(event);
            if (!pointerIsDown) {
        ...
        /** Mouse Events End */
        /** Touch Events Start */
        this.__glcanvas.addEventListener('touchstart', (event: any) => {
            event.stopPropagation();
            prepareEvent(event);
        ...
        this.__glcanvas.addEventListener('touchend', (event: any) => {
            event.stopPropagation();
            prepareEvent(event);
        ...
        this.__glcanvas.addEventListener('touchmove', (event: any) => {
            event.stopPropagation();
            prepareEvent(event);
            for (let i = 0; i < event.touches.length; i++) {
                calcRendererCoords(event.touches[i]);
        ...
        if (window.addEventListener)
            ...
            /** IE/Opera. */
            window.onmousewheel = (document as any).onmousewheel = onWheel;
        }
     ...
     */
    getOrCreateShader(shaderName: any) {
     ...
     * @return {number} - The return value.
     */
    addPass(pass: any, passtype = 0, updateIndices = true) {
        ...
        if (updateIndices) {
            ...
            for (const key in this.__passes) {
                ...
                passSet.forEach((pass: any, index: any) => {
     ...
     */
    registerPass(itemAddedFn: any, itemRemovedFn: any) {
    ...
    __setupXRViewport() {
        ...
        const emitViewChanged = (event: any) => {
     ...
     */
    bindGLBaseRenderer(renderState: any) {
        ...
        if (!renderState.viewports || renderState.viewports.length == 1) {
            renderState.bindRendererUnifs = (unifs: any) => {
                ...
                if (eye) {
                    ...
                    // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'index'.
                    gl.uniform1i(eye.location, index);
                }
            };
            renderState.bindViewports = (unifs: any, cb: any) => cb();
        }
        else {
            renderState.bindRendererUnifs = (unifs: any) => {
            ...
            renderState.bindViewports = (unifs: any, cb: any) => {
                renderState.viewports.forEach((vp: any, index: any) => {
     ...
     * @param {object} renderState - The renderState value.
     */
    drawHighlightedGeoms(renderState: any) {
     ...
     */
    drawSceneGeomData(renderState: any, mask = 255) {
        this.bindGLBaseRenderer(renderState);
        for (const key in this.__passes) {
