...
class VLAAsset extends AssetItem {
   ...
   */
  loadDataFile(onDone: any, onGeomsDone: any) {
    ...
    resourceLoader.loadFile('archive', url).then((entries: any) => {
    ...
    const loadGeomsfile = (geomFileUrl: any) => {
    ...
    this.__geomLibrary.on('streamFileParsed', (event: any) => {
   ...
   * @param {function} onDone - The onDone value.
   */
  fromJSON(j: any, context: any, onDone: any) {
    if (!context) context = {}
