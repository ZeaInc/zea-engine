...
class Mesh extends BaseGeom {
    __edgeAttributes: any;
    __faceAttributes: any;
    __faceCounts: any;
    __faceVertexIndices: any;
    __logTopologyWarnings: any;
    edgeAngles: any;
    edgeFaces: any;
    edgeVecs: any;
    edgeVerts: any;
    ...
    setFaceCounts(faceCounts: any) {
        ...
        this.__faceAttributes.forEach((attr: any) => {
     ...
     * @return {number} - The return value.
     */
    getFaceVertexCount(faceIndex: any) {
        let idx = 0;
        let count = 0;
     ...
     * @return {number} - The return value.
     */
    getFaceVertexOffset(faceIndex: any) {
        let idx = 0;
        let offset = 0;
        this.__faceCounts.some((fc: any, index: any) => {
            if (idx + fc > faceIndex) {
     ...
     */
    setFaceVertexIndices(faceIndex: any, vertexIndices: any) {
     ...
     */
    getFaceVertexIndices(faceIndex: any) {
        const vertexIndices = [];
        const offset = this.getFaceVertexOffset(faceIndex);
     ...
     */
    getFaceVertexIndex(faceIndex: any, facevertex: any) {
        const offset = this.getFaceVertexOffset(faceIndex);
     ...
     */
    // @ts-expect-error ts-migrate(2416) FIXME: Property 'addVertexAttribute' in type 'Mesh' is no... Remove this comment to see the full error message
    addVertexAttribute(name: any, dataType: any, defaultScalarValue = undefined) {
     ...
     */
    addFaceAttribute(name: any, dataType: any, count = undefined) {
     ...
     */
    addEdgeAttribute(name: any, dataType: any, count = undefined) {
    ...
    computeVertexNormals(hardAngle = 1.0 /* radians */) {
        ...
        const setVertexNormal = (index: any, value: any) => {
    ...
    getWireframeIndices() {
        ...
        // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'indices'.
        return indices;
    }
    // ////////////////////////////////////////
    // Rendering
     ...
     */
    generateTriangulatedIndices(totalNumVertices: any, numUnSplitVertices: any, splitIndices: any) {
    ...
    fromJSON(j: any, context: any) {
        ...
        if (j.faceCounts)
            this.__faceCounts = j.faceCounts;
