...
class Attribute {
   ...
   */
  constructor(dataType: any, expectedSize: any, defaultValue = undefined) {
   ...
   */
  setFloat32Value(index: any, value: any) {
   ...
   */
  fromJSON(j: any) {
    const data = j.data.map((dataElement: any) => MathFunctions.isNumeric(dataElement) ? dataElement : Number.POSITIVE_INFINITY
    )
