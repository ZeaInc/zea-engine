...
class VertexAttribute extends Attribute {
     ...
     */
    getFaceVertexValueRef(face: any, facevertex: any) {
     ...
     */
    setFaceVertexValue(face: any, facevertex: any, value: any) {
        const vertex = this.__geom.getFaceVertexIndex(face, facevertex);
     ...
     * @param {AttrValue} value - The value value.
     */
    setFaceVertexValue_ByVertexIndex(face: any, vertex: any, value: any) {
     ...
     * @param {AttrValue} value - The value value.
     */
    setSplitVertexValue(vertex: any, face: any, value: any) {
        if (!(vertex in this.__splits))
            this.__splits[vertex] = {};
        if (face in this.__splits[vertex]) {
     ...
     * @param {AttrValue} value - The value value.
     */
    setSplitVertexValues(vertex: any, faceGroup: any, value: any) {
        if (!(vertex in this.__splits))
            this.__splits[vertex] = {};
     ...
     */
    generateSplitValues(splitIndices: any, splitCount: any) {
        if (splitCount == 0)
            return this.__data;
            ...
            for (const face in faces) {
                ...
                if (vertex in this.__splits && face in this.__splits[vertex]) {
                    ...
                    if (this.__dataType == Float32)
                        ...
                        this.__dataType.createFromBuffer(data.buffer, tgt * numElems * 4).setFromOther(this.__splitValues[src]);
                }
                else {
     ...
     * @param {object} context - The context value.
     */
    // @ts-expect-error ts-migrate(2416) FIXME: Property 'fromJSON' in type 'VertexAttribute' is n... Remove this comment to see the full error message
