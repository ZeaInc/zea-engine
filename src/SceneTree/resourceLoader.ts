...
class ResourceLoader extends EventEmitter {
   ...
   * @private
   */
  loadURL(resourceId: any, url: any, callback: any, addLoadWork = true) {
   ...
   */
  loadCommonAssetResource(resourceId: any) {
    if (resourceId in this.__commonResources) {
      return this.__commonResources[resourceId]
    }
    // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'VLAAsset'.
   ...
   */
  addWork(resourceId: any, amount: any) {
   ...
   * @param {number} amount - The amount value.
   */
  addWorkDone(resourceId: any, amount: any) {
