...
class AssetItem extends TreeItem {
    ...
    readBinary(reader: any, context = {}) {
        (context as any).assetItem = this;
        (context as any).numTreeItems = 0;
        (context as any).numGeomItems = 0;
        if (!(context as any).versions['zea-engine']) {
            (context as any).versions['zea-engine'] = new Version(reader.loadStr());
        }
        this.__engineDataVersion = (context as any).versions['zea-engine'];
        console.log('Loading Engine File version:', (context as any).versions['zea-engine']);
        let layerRoot: any;
        const layers = {};
        (context as any).addGeomToLayer = (geomItem: any, layer: any) => {
        ...
        if ((context as any).versions['zea-engine'].compare([0, 0, 6]) > 0) {
        ...
        if ((context as any).versions['zea-engine'].compare([0, 0, 5]) >= 0 &&
            (context as any).versions['zea-engine'].compare([0, 0, 7]) < 0) {
            loadUnits();
        }
     ...
     */
    toJSON(context = {}) {
        (context as any).makeRelative = (path: any) => {
     ...
     */
    // @ts-expect-error ts-migrate(2416) FIXME: Property 'fromJSON' in type 'AssetItem' is not ass... Remove this comment to see the full error message
    fromJSON(j: any, context = {}, onDone: any) {
        ...
        (context as any).numTreeItems = 0;
        (context as any).numGeomItems = 0;
        if ((context as any).version == undefined)
            (context as any).version = 0;
        (context as any).assetItem = this;
        const plcbs: any = []; // Post load callbacks.
        (context as any).resolvePath = (path: any, cb: any) => {
            ...
            if (!path)
                throw new Error('Path not spcecified');
        ...
        (context as any).addPLCB = (plcb: any) => plcbs.push(plcb);
