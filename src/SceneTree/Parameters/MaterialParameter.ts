...
class MaterialParameter extends Parameter {
  ...
  __valueParameterValueChanged(event: any) {
   ...
   */
  // @ts-expect-error ts-migrate(2416) FIXME: Property 'toJSON' in type 'MaterialParameter' is n... Remove this comment to see the full error message
  toJSON(context: any) {
    let j
    if (this.__value) {
      j = {
  ...
  fromJSON(j: any, context: any) {
    ...
    if (j.value instanceof Array || j.value instanceof String) {
      if (context && context.assetItem) {
        ...
        // @ts-expect-error ts-migrate(2552) FIXME: Cannot find name 'array'. Did you mean 'Array'?
   ...
   */
  // @ts-expect-error ts-migrate(2416) FIXME: Property 'clone' in type 'MaterialParameter' is no... Remove this comment to see the full error message
  clone() {
