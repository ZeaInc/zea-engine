...
class ListParameter extends Parameter {
   ...
   */
  constructor(name: any, dataType: any) {
   ...
   */
  setElement(index: any, value: any) {
   ...
   * @param {string|Parameter} elem - The elem value.
   */
  insertElement(index: any, elem: any) {
    if (!this.__filter(elem)) return
   ...
   */
  // @ts-expect-error ts-migrate(2416) FIXME: Property 'toJSON' in type 'ListParameter' is not a... Remove this comment to see the full error message
  toJSON(context: any) {
    const items = []
   ...
   */
  // @ts-expect-error ts-migrate(2416) FIXME: Property 'clone' in type 'ListParameter' is not as... Remove this comment to see the full error message
  clone() {
