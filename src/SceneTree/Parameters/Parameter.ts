...
class Parameter extends EventEmitter {
   ...
   */
  constructor(name: any, value: any, dataType: any) {
    super()
   ...
   */
  bindOperatorOutput(operatorOutput: any, index = -1) {
   ...
   */
  unbindOperator(operatorOutput: any) {
   ...
   */
  setCleanFromOp(value: any, index: any) {
    if (index != this.__dirtyOpIndex) {
      if (index < this.__dirtyOpIndex) {
  ...
  clone() {
    ...
    // @ts-expect-error ts-migrate(2588) FIXME: Cannot assign to 'clonedValue' because it is a con... Remove this comment to see the full error message
