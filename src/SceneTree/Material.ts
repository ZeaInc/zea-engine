...
const generateParameterInstance = (paramName: any, defaultValue: any, range: any, texturable: any) => {
...
class Material extends BaseItem {
     ...
     */
    constructor(name: any, shaderName: any) {
        super(name);
     ...
     * @private
     */
    __makeParameterTexturable(param: any) {
        // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'makeParameterTexturable'.
     ...
     * @param {string} shaderName - The shader name.
     */
    modifyParams(paramValues: any, shaderName: any) {
        if (shaderName)
            this.setShaderName(shaderName);
    ...
    toJSON(context: any) {
        ...
        (j as any).shader = this.__shaderName;
        return j;
    }
    /**
