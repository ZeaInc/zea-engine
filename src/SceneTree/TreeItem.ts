...
// @ts-expect-error ts-migrate(2345) FIXME: Argument of type '"#03E3AC"' is not assignable to ... Remove this comment to see the full error message
let selectionOutlineColor = new Color('#03E3AC');
selectionOutlineColor.a = 0.1;
// @ts-expect-error ts-migrate(2345) FIXME: Argument of type '"white"' is not assignable to pa... Remove this comment to see the full error message
...
class TreeItem extends BaseItem {
     ...
     */
    static setSelectionOutlineColor(color: any) {
     ...
     * @param {Color} color - The color value.
     */
    static setBranchSelectionOutlineColor(color: any) {
    ...
    setOwner(parentItem: any) {
        if (this.__ownerItem) {
            ...
            if (index >= 0)
                this.__ownerItem.__unbindChild(index, this);
        }
     ...
     */
    addHighlight(name: any, color: any, propagateToChildren = false) {
     ...
     * @param {boolean} propagateToChildren - A boolean indicating whether to propagate to children.
     */
    removeHighlight(name: any, propagateToChildren = false) {
        if (name in this.__highlightMapping) {
            const id = this.__highlights.indexOf(name);
            this.__highlights.splice(id, 1);
     ...
     */
    insertChild(childItem: any, index: any, maintainXfo = false, fixCollisions = true) {
     ...
     */
    addChild(childItem: any, maintainXfo = true, fixCollisions = true) {
     ...
     * @private
     */
    __unbindChild(index: any, childItem: any) {
     ...
     */
    removeChildByHandle(childItem: any) {
     ...
     */
    resolvePath(path: any, index = 0, displayError = false) {
     ...
     */
    traverse(callback: any, includeThis = true) {
    ...
    toJSON(context: any) {
        ...
        if (Object.keys(childItemsJSON).length > 0) {
            if (j) {
                ...
                // @ts-expect-error ts-migrate(2588) FIXME: Cannot assign to 'j' because it is a constant.
                j = {
                    name: this.__name,
