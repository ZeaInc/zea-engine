...
class VLHImage extends BaseImage {
     ...
     * @private
     */
    __loadVLH(fileId: any, file: any) {
        this.type = 'FLOAT';
        resourceLoader.loadFile('archive', file.url).then((entries: any) => {
     ...
     */
    // @ts-expect-error ts-migrate(2416) FIXME: Property 'toJSON' in type 'VLHImage' is not assign... Remove this comment to see the full error message
    toJSON(context: any) { }
    /**
        ...
        if (typeof resourcePath === 'string' && resourcePath != '') {
            if (context.lod >= 0) {
                ...
                if (suffixSt != -1) {
                    ...
                    if ((resourceLoader as any).resourceAvailable(lodPath)) {
                        // @ts-expect-error ts-migrate(2588) FIXME: Cannot assign to 'resourcePath' because it is a co... Remove this comment to see the full error message
