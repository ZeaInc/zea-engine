...
class LabelManager extends EventEmitter {
    __foundLabelLibraries: any;
    __labelLibraries: any;
    __language: any;
    /**
    ...
    loadLibrary(name: any, url: any) {
        ...
        if (name.endsWith('.labels')) {
        ...
        else if (name.endsWith('.xlsx') && window.navigator && (window as any).XLSX) {
            ...
            loadBinfile(url, (data: any) => {
                ...
                // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'XLSX'.
     ...
     */
    getLabelText(libraryName: any, labelName: any) {
        ...
        if (!labelText) {
            if (label['En'])
                return label['En'];
            // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'language'.
     ...
     */
    setLabelText(libraryName: any, labelName: any, labelText: any) {
