...
class VideoStreamImage2D extends BaseImage {
     ...
     */
    connectWebcam(width: any, height: any, rearCamera = false) {
        const video = {
            width,
            height,
            frameRate: {
     ...
     */
    // @ts-expect-error ts-migrate(2416) FIXME: Property 'getParams' in type 'VideoStreamImage2D' ... Remove this comment to see the full error message
    getParams() {
        return {
            type: this.type,
            format: this.format,
