...
// Note: when u == 0.5 z = 1
function dirFromLatLongUVs(u: any, v: any) {
    // http://gl.ict.usc.edu/Data/HighResProbes/
    const theta = Math.PI * (u * 2 - 1);
    const phi = Math.PI * v;
    // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'sin'.
    return new Vec3(sin(phi) * sin(theta), -sin(phi) * cos(theta), cos(phi));
}
function dirToSphOctUv(normal: any) {
...
function sphOctUvToDir(uv: any) {
    ...
    if (sabsuv > 1.0) {
        // @ts-expect-error ts-migrate(2348) FIXME: Value of type 'typeof Vec2' is not callable. Did y... Remove this comment to see the full error message
        const ts = Vec2(uv.y, uv.x);
        uv = (abs_vec2(ts) as any).negate().add(new Vec2(1, 1)).multiply(suv);
        // @ts-expect-error ts-migrate(2588) FIXME: Cannot assign to 'sabsuv' because it is a constant... Remove this comment to see the full error message
