...
class Camera extends TreeItem {
  __farParam: any;
  __focalDistanceParam: any;
  __fovParam: any;
  __isOrthographicParam: any;
  __nearParam: any;
  /**
  ...
  constructor(name = undefined) {
    // @ts-expect-error ts-migrate(2322) FIXME: Type '"Camera"' is not assignable to type 'undefin... Remove this comment to see the full error message
    ...
    const emitProjChanged = (event: any) => {
  ...
  setLensFocalLength(value: any) {
    ...
    // @ts-expect-error ts-migrate(2360) FIXME: The left-hand side of an 'in' expression must be o... Remove this comment to see the full error message
    if (!value in mapping) {
   ...
   */
  setPositionAndTarget(position: any, target: any) {
   ...
   */
  frameView(viewport: any, treeItems: any) {
   ...
   */
  updateProjectionMatrix(mat: any, aspect: any) {
