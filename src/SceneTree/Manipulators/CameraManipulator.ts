...
class CameraManipulator extends BaseTool {
  __defaultManipulationState: any;
  __dollySpeedParam: any;
  __dragging: any;
  __focusIntervalId: any;
  __keyboardMovement: any;
  __keysPressed: any;
  __manipMode: any;
  __manipulationState: any;
  __mouseWheelDollySpeedParam: any;
  __mouseWheelZoomIntervalId: any;
  __ongoingTouches: any;
  __orbitRateParam: any;
  __pointerDown: any;
  __prevPointerPos: any;
  __prevVelocityIntegrationTime: any;
  __velocity: any;
  appData: any;
  prevCursor: any;
  tumbler: any;
   ...
   */
  setDefaultManipulationMode(manipulationMode: any) {
   ...
   * @param {Vec2} dragVec - The drag vector value.
   */
  turntable(event: any, dragVec: any) {
    const { viewport } = event
    const camera = viewport.getCamera()
   ...
   * @param {Vec2} dragVec - The drag vector value.
   */
  trackball(event: any, dragVec: any) {
    const { viewport } = event
    const camera = viewport.getCamera()
    const focalDistance = camera.getFocalDistance()
    const orbitRate = this.__orbitRateParam.getValue()
   ...
   */
  panAndZoom(event: any, panDelta: any, dragDist: any) {
    const { viewport } = event
    const camera = viewport.getCamera()
   ...
   */
  aimFocus(camera: any, target: any, distance = -1, duration = 400) {
   ...
   * @param {Number} duration - The duration in milliseconds to aim the focus.
   */
  orientPointOfView(camera: any, position: any, target: any, distance = 0, duration = 400) {
    if (this.__focusIntervalId) clearInterval(this.__focusIntervalId)
   ...
   */
  onPointerDoublePress(event: any) {
   ...
   * @private
   */
  integrateVelocityChange(event: any) {
    const { viewport } = event
    const camera = viewport.getCamera()
