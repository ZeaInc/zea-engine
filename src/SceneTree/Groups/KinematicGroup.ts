...
class KinematicGroup extends BaseGroup {
  __initialXfoModeParam: any;
  calculatingGroupXfo: any;
  groupTransformOp: any;
  memberXfoOps: any;
  /**
  ...
  calcGroupXfo() {
    ...
    this.memberXfoOps.forEach((op: any) => op.disable())
    ...
    this.memberXfoOps.forEach((op: any) => op.enable())
    this.calculatingGroupXfo = false
  }
   ...
   */
  setItems(items: any) {
    // @ts-expect-error ts-migrate(2663) FIXME: Cannot find name 'emit'. Did you mean the instance... Remove this comment to see the full error message
    super.setItems(emit)
    this.calcGroupXfo()
  }
