...
class Mat4 extends AttrValue {
   ...
   */
  setLookAt(pos: any, target: any, up: any) {
   ...
   */
  transformVec4(vec: any) {
    const a = this.__data
    const x = vec.x
    const y = vec.y
    const z = vec.z
    const w = vec.t
    // @ts-expect-error ts-migrate(2552) FIXME: Cannot find name 'Vec4'. Did you mean 'vec'?
    return new Vec4(
   ...
   */
  setPerspectiveMatrix(fovy: any, aspect: any, near: any, far: any) {
   ...
   * @param {number} far - The far value.
   */
  setOrthographicMatrix(left: any, right: any, bottom: any, top: any, near: any, far: any) {
