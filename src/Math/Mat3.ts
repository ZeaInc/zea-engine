...
class Mat3 extends AttrValue {
  ...
  inverse() {
    ...
    // @ts-expect-error ts-migrate(2588) FIXME: Cannot assign to 'det' because it is a constant.
    det = 1.0 / det
   ...
   */
  transpose() {
    // @ts-expect-error ts-migrate(2348) FIXME: Value of type 'typeof Mat3' is not callable. Did y... Remove this comment to see the full error message
    return Mat3(
      this.__data[0],
      this.__data[3],
      this.__data[6],
  ...
  clone() {
    return new Mat3(
      ...
      this.__data[7],
      this.__data[8],
      // @ts-expect-error ts-migrate(2554) FIXME: Expected 0-9 arguments, but got 10.
      this.__data[9]
    )
  }
