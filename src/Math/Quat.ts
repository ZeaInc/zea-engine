...
class Quat extends AttrValue {
  ...
  constructor(x = 0, y = 0, z = 0, w = 1) {
    ...
    // @ts-expect-error ts-migrate(2358) FIXME: The left-hand side of an 'instanceof' expression m... Remove this comment to see the full error message
      ...
      if (typeof x === 'object') {
        ...
        this.__data[2] = 0
        this.__data[3] = 1
        // @ts-expect-error ts-migrate(2407) FIXME: The right-hand side of a 'for...in' statement must... Remove this comment to see the full error message
        for (const key in x) {
   ...
   */
  setFromEulerAngles(eulerAngles: any) {
   ...
   */
  toEulerAngles(rotationOrder: any) {
    const ordered = new Vec3()
   ...
   */
  setFromAxisAndAngle(axis: any, angle: any) {
   ...
   */
  setFromDirectionAndUpvector(dir: any, up: any) {
    const mat3 = new Mat3()
   ...
   * @return {boolean} - Returns true or false.
   */
  approxEqual(other: any, precision = Number.EPSILON) {
    return (
  ...
  slerp(other: any, t: any) {
    ...
    // @ts-expect-error ts-migrate(2588) FIXME: Cannot assign to 'theta' because it is a constant.
   ...
   * @private
   */
  static createFromFloat32Buffer(buffer: any, offset = 0) {
   ...
   * @return {Quat} - Returns a new Quat.
   */
  static createFromBuffer(buffer: any, byteOffset: any) {
    // @ts-expect-error ts-migrate(2345) FIXME: Argument of type 'Float32Array' is not assignable ... Remove this comment to see the full error message
