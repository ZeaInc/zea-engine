...
class GrowingPacker extends EventEmitter {
  ...
  // @ts-expect-error ts-migrate(7023) FIXME: 'findNode' implicitly has return type 'any' becaus... Remove this comment to see the full error message
  findNode(root: any, w: any, h: any) {
    if (root.used) return this.findNode(root.right, w, h) || this.findNode(root.down, w, h)
    else if (w <= root.w && h <= root.h) return root
    else return null
  }

  splitNode(node: any, w: any, h: any) {
    node.used = true
    node.down = {
      x: node.x,
      y: node.y + h,
